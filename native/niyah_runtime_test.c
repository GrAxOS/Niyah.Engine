...
int main(void) {
    ...
    NiyahRuntimeNode *query = niyah_runtime_add_node(
        &graph, NIYAH_GRAPH_SYSTEM, "query", 0, 1);
    NiyahRuntimeNode *source = niyah_runtime_add_node(
        &graph, NIYAH_GRAPH_SOURCE, "source", query->id, 2);
    ...
    NiyahRuntimeEdge *edge = niyah_runtime_add_edge(
        &graph, query->id, source->id, NIYAH_EDGE_RELATES, 0.75f);
    assert(edge != NULL);
    assert(graph.edge_count == 1);
    assert(niyah_runtime_find_node(&graph, source->id) == source);

    assert(niyah_runtime_add_edge(
        &graph, query->id, 99999, NIYAH_EDGE_RELATES, 1.0f) == NULL);
    assert(niyah_runtime_add_edge(
        &graph, query->id, query->id, NIYAH_EDGE_RELATES, 1.0f) == NULL);
