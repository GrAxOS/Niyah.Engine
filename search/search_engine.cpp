...
std::size_t SearchEngine::crawl_once() {
    ...
    if (fetched_documents_ >= config_.max_documents) {
        if (telemetry_ready_) (void)niyah_telemetry_event(&telemetry_, "crawl_limit", elapsed_ms(start), 0, 0, false);
        return 0;
    }

    NiyahCrawlItem item{};
    if (!niyah_crawl_pop(&frontier_, &item)) {
        if (telemetry_ready_) (void)niyah_telemetry_event(&telemetry_, "crawl_empty", elapsed_ms(start), 0, 0, false);
        return 0;
    }
    if (item.depth > config_.max_depth) {
        if (telemetry_ready_) (void)niyah_telemetry_event(&telemetry_, "crawl_depth_blocked", elapsed_ms(start), 0, 0, false);
        return 0;
    }
    ...
    if (!niyah_index_add_document(&index_, &indexed)) {
        if (telemetry_ready_) (void)niyah_telemetry_event(&telemetry_, "index_failed", elapsed_ms(start), fetched.body.size(), 0, true);
        return 0;
    }
